# Coding-Agent-Werkzeuge und Arbeitsumgebung

## Ziel

Die Teilnehmer kennen die noetige Arbeitsumgebung fuer Coding-Agents und koennen einschaetzen, welcher Einstiegspfad fuer das eigene Geraet passt.

## Anbieter- und Agent-Zugang

Geeignet fuer:

- kleine Softwareprojekte
- Erklaerungen zu Code
- Aenderungen in Projektdateien
- Fehlersuche
- Tests und Verbesserungen

Vorteile:

- der Coding-Agent arbeitet im Projektkontext
- Aenderungen koennen direkt ausprobiert werden
- Aufgaben koennen schrittweise formuliert werden

Noetig:

- Subscription oder ausreichend Credits bei einem Modellanbieter
- ausreichendes Nutzungskontingent fuer mehrere Kursstunden, zum Beispiel bei Claude, OpenAI, Qwen oder einem vergleichbaren Anbieter
- fuer den Workshopzeitraum besser ein Pro-, Max- oder vergleichbarer Einzeltarif mit hoeherem Limit
- grobe Budgetorientierung: ca. 100 EUR pro Monat, je nach Anbieter, Abrechnung und aktuellem Tarif
- passender Coding-Agent, der mit diesem Anbieter arbeitet

Der Coding-Agent allein reicht nicht; er braucht einen funktionierenden Modellzugang. Niedrige Plus- oder Standard-Tarife koennen fuer laengere Agent-Sessions zu schnell an Token- und Nutzungslimits stossen. Preise und Limits sollten vor dem Kurs beim jeweiligen Anbieter geprueft werden.

Grenzen:

- Konto, Subscription oder Nutzungskontingent muessen vorliegen
- Datenschutzbedingungen beachten
- Ergebnisse muessen geprueft werden
- groessere Projekte brauchen mehr Struktur

## Lokale Werkzeuge im Kurs

Moegliche Bestandteile:

- Visual Studio Code oder ein anderer Editor
- Browser mit Entwicklertools
- Terminal
- Coding-Agent-Werkzeug, zum Beispiel Codex CLI, Claude Code, Pi Agent oder ein vergleichbares Werkzeug
- optional Git fuer spaetere Vertiefung
- optional Node.js oder Python fuer spaetere Projekte

Vorteile:

- Dateien bleiben lokal
- Projekte sind besser wiederverwendbar
- echte Entwicklungsablaeufe werden sichtbar

Grenzen:

- Installation kann Zeit kosten
- Fehlermeldungen wirken anfangs ungewohnt
- Betriebssysteme unterscheiden sich

## Gemeinsamer Projektpfad

Fuer den Einstieg eignet sich ein einfaches Browser-Projekt:

- `index.html` fuer Inhalt
- `style.css` fuer Gestaltung
- `script.js` fuer Interaktion

Damit kann ein Ergebnis ohne Server direkt im Browser geoeffnet werden.

## Kursregel

Die Kurszeit wird nicht mit komplizierter Installation verbraucht. Agent-Zugang und Basiswerkzeuge werden moeglichst vorab vorbereitet. Wenn ein Geraet nicht rechtzeitig eingerichtet ist, wird mit einem gemeinsamen Demo-Projekt gearbeitet.
